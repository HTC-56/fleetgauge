...
2. Gate: `gofmt -l .` prints nothing AND `go vet ./...` clean AND
   `go test ./...` green AND `bash scripts/scrub-check.sh` green (once it
   exists) — always. Red = not done.
3. If green: mark the task "- [x]" in TODO.md, run `git status --short`, stage every
   SOURCE file you created or changed this session (new files, go.mod, go.sum),
   commit with a message naming the task. Never stage generated output
   (built binaries, coverage files, logs, ledger.jsonl).
...
- **This repo will be PUBLIC.** Never write a private hostname, a real LAN IP, an
  ...
  `localhost` and `192.0.2.x` addresses only. The public HTC-56 repos
  (local-ai-gateway, worklane, tenant-kernel) may be named.
- Dependency surface is exactly: the Go standard library + `gopkg.in/yaml.v3`.
  Nothing else, ever, unless your task names it.
- `exec.Command` may appear ONLY inside the systemd backend package. Tests run
  against the fake backend; **no test may shell out or touch real systemd.**
- The page is one self-contained HTML file with zero external requests. No
  frameworks, no build step, no CDN, no web fonts — anywhere in the repo.
- Read-only by default: the only mutating verb in the whole product is the
  triple-gated restart. Do not add others.
